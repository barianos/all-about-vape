      ...
      <v-card flat class="pa-3">
        ...
        <div v-else>
          <v-expansion-panels v-model="expandedPanels" multiple flat>
            <v-expansion-panel
              v-for="(values, filter) in filterOptions"
              :key="filter"
            ...
            >
              <v-expansion-panel-title class="py-2 font-weight-bold">
                ...
                <template v-slot:actions="{ expanded }">
                  <v-badge
                    v-if="selectedFilters[filter]?.length"
                    :content="selectedFilters[filter]?.length"
                    color="primary"
                    class="ms-2"
                  ></v-badge>
              ...
              <v-expansion-panel-text>
                <v-list density="compact" class="filter-list pa-0">
                  <v-list-item
                    v-for="value in values"
                    :key="value"
                    :active="isValueSelected(filter, value)"
                    @click="toggleSelection(filter, value, !isValueSelected(filter, value))"
                    class="py-1 px-2"
                    active-color="primary"
                    rounded="sm"
                  >
                    <template v-slot:prepend>
                      <v-icon v-if="isValueSelected(filter, value)" 
                        color="primary" size="small" icon="mdi-check"
                        class="me-2"></v-icon>
